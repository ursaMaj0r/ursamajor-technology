...
<h2 class="drac-heading drac-text-center drac-heading-xl drac-text-orange">Setting Up Clockify</h2>
...
<h2 class="drac-heading drac-text-center drac-heading-xl drac-text-orange">Installing the Module</h2>
...
```
iex ('$module="";$user="UrsaMaj0r";$repo="PSClockify"'+(new-object net.webclient).DownloadString('https://raw.githubusercontent.com/PsModuleInstall/InstallFromGithub/master/install.ps1'))  

Import-Module PSClockify
```
<br><img src="/assets/images/posts/psclockify/install.png" width="75%" />

<h2 class="drac-heading drac-text-center drac-heading-xl drac-text-orange">Using the Module</h2>
...
```
Connect-Session -APIKey 'api_key_here' -Workspace 'MyAwesomeWorkspace'
```
...
```
New-Project -name 'Chores' -note 'A project to track weekly chores around the house'
```

And we can't forget to add some tasks to the project:
```
New-Task -name Dishes -Project Chores
New-Task -name Trash -Project Chores
New-Task -name Laundry -Project Chores
```
...
```
 Start-Timer -project Chores -description 'Too many socks!!!' -task Laundry
```
...
> Examples:
```
Get-Client -Identity "Calli" -OutputType Detailed
Set-Timer -RestartTimer
New-Tag -Name "Chores"
```
